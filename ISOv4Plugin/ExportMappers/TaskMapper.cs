...
namespace AgGateway.ADAPT.ISOv4Plugin.ExportMappers
{
    ...
    {
        IEnumerable<TSK> Map(IEnumerable<LoggedData> loggedData, Catalog catalog, string datacardPath, int numberOfExistingTasks, TaskDocumentWriter taskDocumentWriter);
    }
    ...
    public class TaskMapper : ITaskMapper
    {
        ...
        public IEnumerable<TSK> Map(IEnumerable<LoggedData> loggedData, Catalog catalog, string datacardPath, int numberOfExistingTasks, TaskDocumentWriter taskDocumentWriter)
        {
            if (loggedData == null)
                yield break;

            var loggedDataList = loggedData.ToList();
            for (var i = 0; i < loggedDataList.Count; ++i)
            {
                yield return Map(loggedDataList[i], catalog, datacardPath, numberOfExistingTasks + (i + 1), taskDocumentWriter);
            }
        }
